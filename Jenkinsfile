        ...
        stage('Deploy to QA') {
            ...
            steps {
                sh '''
                    ...
                    sed "s|<IMAGE_TAG>|${TAG}|g; s|<REPO_NAME>|${REPO_NAME}|g; s|<ENV>|${params.ENVIRONMENT}|g" central-config/k8s-deployment.yaml | \
                ...
                withCredentials([usernamePassword(credentialsId: 'dockerhub-creds', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
                    script {
                        ...
                        sh """
                            echo "${DOCKER_PASS}" | docker login -u "${DOCKER_USER}" --password-stdin
                            docker pull ${qaImage}
                            docker tag ${qaImage} drdocker108/${env.REPO_NAME}:${prodTag}
                            docker push drdocker108/${env.REPO_NAME}:${prodTag}
                            docker logout
                        """
                    }
                }
        ...
        stage('Deploy to Prod') {
            ...
            steps {
                sh '''
                    ...
                    sed "s|<IMAGE_TAG>|${REPO_NAME}-${BUILD_NUMBER}|g; s|<REPO_NAME>|${REPO_NAME}|g; s|<ENV>|${params.ENVIRONMENT}|g" central-config/k8s-deployment.yaml | \
                    kubectl --kubeconfig=${KUBECONFIG} apply -n ${params.ENVIRONMENT} -f -
                '''
            }
        }
